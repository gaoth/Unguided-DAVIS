...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        ...
        if a.lab_colorization:
            ...
            mask = preprocess(raw_input[:,:width//2,:])
            rgb = preprocess(raw_input[:,width//2:,:])
            a_images = tf.expand_dims(mask[:,:,0], axis=2)
            track = tf.expand_dims(mask[:,:,1], axis=2)
            b_images = tf.concat([rgb, track], 2)
